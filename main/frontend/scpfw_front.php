...
function scpfw_header_action() {
    if(is_product()) {
        ...
        if(!empty($chart_ids)) {
            ...
            foreach ($chart_ids as $key => $chart_id) {
                ...
                $alw_gust_usr = get_post_meta( $chart_id, SCPFW_PREFIX.'alw_gust_usr', true );
                ...
                if( $showCouponField == 'true'){
                    if($alw_gust_usr != "on") {
                        ...
                        } elseif ($btn_tab == "sidepopup") {
                            if($btn_pos == "after_add_cart" ) {
                            
                                if ( ! $product->is_in_stock()) {
                                    add_action('woocommerce_product_meta_start','scpfw_button_sidepopup_after',0);
                                } else {
                                    add_action('woocommerce_after_add_to_cart_form', 'scpfw_button_sidepopup_after');
                                }
                                  
                            } elseif ($btn_pos == "before_add_cart") {
                                if( ! $product->is_in_stock()) {
                                    add_action('woocommerce_single_product_summary','scpfw_button_sidepopup_before');
                                } else {
                                    if ( $product->is_type( 'variable' ) ) {
                                        add_action('woocommerce_single_variation','scpfw_button_sidepopup_before');
                                    } else {
                                        add_action('woocommerce_before_add_to_cart_form','scpfw_button_sidepopup_before' );
                                    }
                                }
                            } elseif ($btn_pos == "before_summry_text") {
                                add_action('woocommerce_single_product_summary','scpfw_button_sidepopup_before_summry_text');
                            } elseif($btn_pos == "aftr_prod_meta") {
                                add_action('woocommerce_product_meta_end','scpfw_button_sidepopup_aftr_prod_meta');
                            }
                        }
                    } else {
    ...
    if(is_shop() || is_product_category() || is_product_tag()) {
        add_action('woocommerce_after_shop_loop_item','scpfw_shop_page_popup_button_after', 11);
...
function scpfw_shop_page_popup_button_after() {
    global $post;
                ...
                if($szchartpp_shop_enbl == 'enable') {
                    ...
                    if($alw_gust_usr != "on") {
                        if ($btn_tab == "popup") {
                            ...
                            if($btn_pos == "after_add_cart" ) {
                                ?>
                                <div class="scpfw_sidepoup_btn">
                                    <button class="scpfw_sidepoup_open" data-id="<?php echo esc_attr($post->ID); ?>" data-image="<?php echo esc_attr($loader);?>" data-cid="<?php echo esc_attr($chart_id); ?>" style="<?php echo esc_attr($style); ?>">
                                        <?php echo esc_attr($btn_lbl); ?>
                                    </button>
                                </div>
                                <?php
    ...
    if(!empty($chart_ids)) {
        foreach ($chart_ids as $key => $chart_id) {
            ...
            if($btn_tab == "tab") {
                ...
                $tbl_head_bg_clr        = get_post_meta( $chart_id, SCPFW_PREFIX.'tbl_head_bg_clr', true);
                $tbl_head_ft_clr        = get_post_meta( $chart_id, SCPFW_PREFIX.'tbl_head_ft_clr', true);
                $tbl_even_row_clr       = get_post_meta( $chart_id, SCPFW_PREFIX.'tbl_even_row_clr', true);
                $tbl_odd_row_clr        = get_post_meta( $chart_id, SCPFW_PREFIX.'tbl_odd_row_clr', true);
                $tbl_dtrow_font_clr     = get_post_meta( $chart_id, SCPFW_PREFIX.'tbl_dtrow_font_clr', true);
                ...
                <style type="text/css">
                    ...
                    .scpfw_tableclass tr {
                        color: <?php echo esc_attr($tbl_dtrow_font_clr); ?>;
                    }
                    .scpfw_tableclass tr:nth-child(even) {
                        background: <?php echo esc_attr($tbl_even_row_clr); ?>;
                    }
                    .scpfw_tableclass tr:nth-child(odd) {
                        background: <?php echo esc_attr($tbl_odd_row_clr); ?>;;
                    }
                    .scpfw_tableclass tr:nth-child(1) {
                        background: <?php echo esc_attr($tbl_head_bg_clr); ?>;
                        color: <?php echo esc_attr($tbl_head_ft_clr); ?>;
                        font-weight: 700;
...
function scpfw_button_sidepopup_after() {
    $product_id = get_the_ID();
    $product    = wc_get_product( $product_id );
    $chart_ids = scpfw_get_chart_ids($product_id);

    if(!empty($chart_ids)) {
        scpfw_sideBarSizeChartOpenButton($chart_ids);
    }
}


function scpfw_button_sidepopup_before() {
    $product_id = get_the_ID();
    $product    = wc_get_product( $product_id );
    $chart_ids = scpfw_get_chart_ids($product_id);
...
function scpfw_button_sidepopup_before_summry_text() {
    $product_id = get_the_ID();
    $product    = wc_get_product( $product_id );
    $chart_ids = scpfw_get_chart_ids($product_id);
...
function scpfw_button_sidepopup_aftr_prod_meta() {
    $product_id = get_the_ID();
    $product    = wc_get_product( $product_id );
    $chart_ids = scpfw_get_chart_ids($product_id);
    if(!empty($chart_ids)) {
...
function scpfw_sideBarSizeChartOpenButton($chart_ids){
    if(!empty($chart_ids)) {
        foreach ($chart_ids as $key => $chart_id) {
            $btn_tab      = get_post_meta( $chart_id, SCPFW_PREFIX.'btn_show', true );
            $btn_pos      = get_post_meta( $chart_id, SCPFW_PREFIX.'btn_pos', true );
            if($btn_tab == "sidepopup" || $btn_pos == "aftr_prod_meta") {
                $btn_lbl    = get_post_meta( $chart_id, SCPFW_PREFIX.'btn_lbl', true );
                $btn_ft     = get_post_meta( $chart_id, SCPFW_PREFIX.'btn_ft_clr', true );
                $btn_bg     = get_post_meta( $chart_id, SCPFW_PREFIX.'btn_bg_clr', true );
                $btn_brd_rd = get_post_meta( $chart_id, SCPFW_PREFIX.'btn_brd_rd', true );
                ...
                ?>
                <div class="scpfw_sidepoup_btn">
                    <button class="scpfw_sidepoup_open" data-id="<?php echo esc_attr($product_id); ?>" data-cid="<?php echo esc_attr($chart_id); ?>" data-image="<?php echo esc_attr($loader);?>" style="<?php echo esc_attr($style); ?>">
                        <?php echo esc_attr($btn_lbl); ?>
                    </button>
                </div>
                <?php
            ...
            if( $showCouponField == 'true'){
                ...
                if($alw_gust_usr != "on") {
                    if ($btn_tab == "popup") {
                            ...
                            ?>
                            <div class="scpfw_sidepoup_btn">
                                <button class="scpfw_sidepoup_open" data-id="<?php echo esc_attr($pro_id); ?>" data-image="<?php echo esc_attr($loader);?>" data-cid="<?php echo esc_attr($chart_id); ?>" style="<?php echo esc_attr($style); ?>">
                                    <?php echo esc_attr($btn_lbl); ?>
                                </button>
                            </div>
                            <?php
